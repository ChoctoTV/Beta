...
// ── Choctopus-themed battle commentary (Rumble-Royale-style) ──────────────────
const FLAVOR = {
  hit: [
    'whips a tentacle at',
    'lobs a half-melted Choctopus bar at',
    'slaps with a peanut-butter-coated paw at',
    'unleashes the Squid-Hug™ on',
    'flings sticks of fury at',
    'goes full tentacle-tornado on',
    'inkblasts',
    'serves a slimy uppercut to',
    'bonks with a frozen pup-sicle on',
    'tail-whips chocolate sauce all over',
    'pelts with stale kibble at',
    'sneezes peanut dust at',
    'bites the ankles of',
    'rolls a soccer ball of doom into',
    'launches a flying bork at',
    'unleashes the Cthulhu-Choctopus combo on',
    'sloshes a bowl of cold water at',
    'bops with a chew toy at',
    'fires a chocolate-coated tentacle at',
    'goes squid-mode and tangles up',
  ],
  hitCrit: [
    'UNLEASHES THE SQUID-SLAP-9000 on',
    'CRITS WITH AN 8-TENTACLE TRIPLE COMBO on',
    'LANDS THE LEGENDARY MOON-SHOT on',
    'GOES FULL DARK-CHOCTO MODE on',
    'PERFECT-CRITS WITH A FROZEN BANANA on',
    'KRAKEN-CRITS',
    'DROPS THE TENTACLE OF JUSTICE on',
    'CHOCTO-BOMBS',
    'SPECIAL MOVE: PEANUT BUTTER PIPE-BOMB on',
    'CHEW-TOY-OF-DESTINY-CRITS',
    'INVOKES THE FORBIDDEN LICK on',
  ],
  kill: [
    'wraps eight tentacles around and DUNKS',
    'feeds the last chocolate to and ENDS',
    'sticks-balls-AND-moons combos and finishes',
    'unleashes the FINAL CHOCTO-BLAST on',
    'sends to the great peanut butter jar in the sky:',
    'YEETS into the kiddie pool — defeating',
    'goes Squid-Sicko mode and ELIMINATES',
    'turns into pure Choctopus rage and KOs',
    'rage-quits the chew toy aimed at',
    'launches a Decemgrrr Drip combo and finishes',
  ],
  killCrit: [
    'PULLS A CAPTAIN SPLORCH FINISHER on',
    'ACTIVATES AUGRUFF MODE and OBLITERATES',
    'INVOKES NEWBARKS DAY VENGEANCE on',
    'SUMMONS THE KRAKEN — destroying',
    'DROPS A MOON-FORGE NUKE on',
    'GOES OCTOBARK HOWLOWEEN on',
    'PERFORMS THE LEGENDARY 8-TENTACLE SUPLEX on',
    'CRITS WITH THE WRATH OF A THOUSAND CHOCTONAUTS on',
  ],
};
function pickFlavor(kind, crit) {
  const pool = FLAVOR[kind + (crit ? 'Crit' : '')];
...
const PuppyWars = {
  ...
  _buildDOM() {
    // Battle area sits ONLY at the bottom 220px of the scene, overlaying the
    // chronicles + calendar. inset:0 would fill the whole screen — don't.
    this.mount.style.cssText = 'position:absolute;bottom:0;left:0;right:0;height:220px;pointer-events:none;display:block;z-index:9000;';
    if (!document.getElementById('pw-css')) {
      ...
      s.textContent = `
        ...
        .pw-duel-name{position:absolute;top:-40px;left:50%;transform:translateX(-50%);font-size:18px;font-weight:800;color:#fff;white-space:nowrap;text-shadow:0 1px 3px #000;}
    ...
    this.container.style.cssText = 'position:absolute;inset:0;background:rgba(0,0,0,.9);border-top:2px solid rgba(255,255,255,.14);overflow:hidden;';
    this.arenaEl = document.createElement('div');
    this.arenaEl.style.cssText = 'height:100%;display:none;background:linear-gradient(180deg,rgba(8,4,18,.97),rgba(15,8,30,.99));border-top:3px solid rgba(231,76,60,.5);box-shadow:0 -8px 32px rgba(0,0,0,.7);';
  ...
  async _runRounds() {
    ...
    while (true) {
      ...
      if (def.hp <= 0) {
        def.alive = false;
        const flavor = pickFlavor('kill', crit);
        b.log.push(crit
          ? `💥 @${atk.user} ${flavor} — @${def.user} ELIMINATED (<b>${dmg}</b>)`
          : `⚔️ @${atk.user} ${flavor} — @${def.user} OUT (<b>${dmg}</b>)`);
      } else {
        const flavor = pickFlavor('hit', crit);
        b.log.push(crit
          ? `💥 @${atk.user} ${flavor} @${def.user} for <b>${dmg}</b>!`
          : `⚔️ @${atk.user} ${flavor} @${def.user} for <b>${dmg}</b>`);
      }
  ...
  async _startGauntlet(entry) {
    ...
    for (const oppSprite of opponents) {
      ...
      while (player.hp > 0 && opp.hp > 0) {
        ...
        const pCrit = pd >= ps.dmax * 0.9;
        opp.hp     = Math.max(0, opp.hp - pd);
        this.battle.log = [`⚔️ @${player.user} ${pickFlavor('hit', pCrit)} ${opp.user} for ${pd}!`];
        this.battle._gAtk = 'player';
        if (opp.hp <= 0) { playerWon = true; this._renderGauntlet(); await delay(ROUND_DELAY); break; }

        // Opponent counter-attacks
        const os   = st(opp.rarity);
        const od   = Math.floor(Math.random()*(os.dmax-os.dmin+1))+os.dmin;
        const oCrit = od >= os.dmax * 0.9;
        player.hp  = Math.max(0, player.hp - od);
        this.battle.log.push(`${opp.user} ${pickFlavor('hit', oCrit)} ${player.user} for ${od}!`);
        ...
        <div style="display:flex;align-items:center;gap:20px;flex:1;">
          ...
          <div style="flex:1;text-align:center;">
            ...
            <div style="background:rgba(255,255,255,.04);border-radius:8px;padding:8px 12px;min-height:60px;border:1px solid rgba(255,255,255,.06);">
              ${b.log.map(l=>`<div style="font-size:20px;color:#fff;line-height:1.7;font-weight:600;text-shadow:0 1px 3px rgba(0,0,0,.8);">${l}</div>`).join('')}
            </div>
    ...
    if (!b._duelBuilt) {
      this.arenaEl.innerHTML = `
        <div style="position:relative;height:100%;overflow:hidden;font-family:'Segoe UI',system-ui,sans-serif;">
          ...
          <div style="position:absolute;bottom:6px;left:0;right:0;text-align:center;font-size:18px;color:#fff;font-weight:600;text-shadow:0 1px 3px rgba(0,0,0,.8);padding:0 10px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;" id="pw-duel-log">${lastLog}</div>
        </div>`;
      b._duelBuilt = true;
    ...
    this.arenaEl.innerHTML = `
      <div style="display:flex;flex-direction:column;height:100%;padding:6px 8px;gap:4px;font-family:'Segoe UI',system-ui,sans-serif;">
        <div style="display:flex;align-items:center;justify-content:space-between;flex-shrink:0;padding:0 4px;">
          ...
          <div style="font-size:18px;color:#fff;font-weight:600;text-shadow:0 1px 3px rgba(0,0,0,.8);max-width:1000px;text-align:right;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">${lastLog}</div>
        </div>
                  ...
                  <div style="flex:0 0 ${cellW-3}px;max-width:${cellW-3}px;background:rgba(255,255,255,.04);border-radius:6px;padding:4px 5px;
                    ...
                    <div style="display:flex;align-items:center;gap:4px;">
                      ...
                      <div style="flex:1;min-width:0;overflow:hidden;">
                        <div style="font-size:13px;font-weight:800;color:${f.alive?'#fff':'#444'};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">@${f.user}</div>

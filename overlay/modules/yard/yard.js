...
const Yard = {
  ...
  async init({mount,EventBus,CONFIG,getRandomSprite,getSpriteByPupId,getSpriteByName,getSpritesByRarity,getImageForSprite}){
    ...
    this._bind();
    // ── Billboard slideshow ─────────────────────────────────────────────────────
    // Images loaded directly from assets/billboard/manifest.json (written by start.sh).
    // Cycles through all images in order. No server endpoint needed.
    this.billboardImg   = null;
    this._billboardAll  = [];
    this._billboardIdx  = 0;
    this._billboardTimer = null;

    this._billboardFetch = async () => {
      try {
        const r = await fetch('./assets/billboard/manifest.json?t=' + Date.now());
        if (!r.ok) { console.warn('[Billboard] manifest.json not found — run start.sh'); return; }
        const j = await r.json();
        const all = [...(Array.isArray(j.main) ? j.main : []),
                     ...(Array.isArray(j.ads)  ? j.ads  : [])];
        this._billboardAll = all;
        console.log('[Billboard] ' + all.length + ' image(s):', all.map(p => p.split('/').pop()).join(', ') || 'none');
        if (!all.length) console.warn('[Billboard] Put images in yardpets3/assets/billboard/main/');
      } catch (e) { console.warn('[Billboard] manifest fetch failed:', e.message); }
    };

    this._billboardLoadImage = src => {
      // Load into the back layer, then crossfade when ready
      const back  = this._bbBack;
      const front = this._bbFront;
      back.onload = () => {
        ...
        setTimeout(() => {
          this._bbFront = back;
          this._bbBack  = front;
          this.billboardImg = true;
        }, 2000);
        console.log('[Billboard] fading to:', src.split('/').pop());
      };
      back.onerror = () => console.warn('[Billboard] FAILED to load:', src);
      back.src = src;
    };

    this._billboardNext = () => {
      if (!this._billboardAll.length) return;
      this._billboardIdx = (this._billboardIdx + 1) % this._billboardAll.length;
      this._billboardLoadImage(this._billboardAll[this._billboardIdx]);
    };

    (async () => {
      await this._billboardFetch();
      if (this._billboardAll.length) this._billboardLoadImage(this._billboardAll[0]);
      const _ms = (this.CONFIG.billboard_interval_sec || 30) * 1000;
      // Persistent loop — advances every 30s, wraps back to start automatically
      this._billboardTimer = setInterval(() => this._billboardNext(), _ms);
      // Re-fetch manifest every 15 min to pick up new files
      setInterval(() => this._billboardFetch(), 15 * 60 * 1000);
    })();
  ...
  _buildDOM(){
    ...
    // Billboard DOM image — sits between sky (z3) and main canvas (z10).
    // Using a real <img> element means animated GIFs play natively.
    // Billboard — two crossfade layers so images dissolve smoothly.
    // Panel coords: bCX=580, BW=520, BH=180, grassY=310, panelT=72
    const _bbStyle = 'position:absolute;left:320px;top:72px;width:520px;height:180px;' +
      'object-fit:fill;z-index:2;image-rendering:auto;background:transparent;' +
      'opacity:0;transition:opacity 2s ease-in-out;pointer-events:none;';
    this._bbLayerA = document.createElement('img');
    this._bbLayerA.style.cssText = _bbStyle;
    this.mount.appendChild(this._bbLayerA);
    this._bbLayerB = document.createElement('img');
    this._bbLayerB.style.cssText = _bbStyle;
    this.mount.appendChild(this._bbLayerB);
    this._bbFront  = this._bbLayerA;  // currently visible layer
    this._bbBack   = this._bbLayerB;  // layer loading next image
    this.billboardImg = false;
    ...
    this.popupLayer=document.createElement('div');this.popupLayer.style.cssText='position:absolute;inset:0;pointer-events:none;z-index:9999;';this.mount.appendChild(this.popupLayer);
  ...
  _tick(dt){
    ...
    if(!this._lastDayEvt||Date.now()-this._lastDayEvt>1000){this._lastDayEvt=Date.now();const _cp=(this._dogSecOfDay()/86400+0.75)%1;const _nt=0.5-0.5*Math.cos((_cp-0.25)*2*Math.PI);try{window.dispatchEvent(new CustomEvent('chocto:daytime',{detail:{cycleP:_cp,night:_nt}}));}catch{}}
    ...
    if (this._skyRoll) {
      this._skyOffset = ((Date.now() - this._skyStartMs) / Math.floor(24*3600000/7) * this._skyRollH) % this._skyRollH;
    }
  ...
  // ── Dog time of day in seconds (0..86399) — shared with ChoctoCalendar ─────
  // Mirrors the formula in services/ChoctoCalendarHelper.js. Used by the sky
  // cycle so day/night phase matches what the in-game calendar shows.
  _dogSecOfDay(){
    const d = new Date();
    ...
    let doy = date - 1;
    for (let m = 0; m < month; m++) doy += GREG[m];
    const humanDays = (year - 1) * 365 + doy + (hh*3600 + mm*60 + ss) / 86400;
    const dogDaysTotal = humanDays * 7;
    const dogDayFrac = dogDaysTotal - Math.floor(dogDaysTotal);
    return Math.floor(dogDayFrac * 86400);
  },
  ...
  _draw(){
    ...
    // One day-cycle clock — derived from the dog time-of-day so the sky and
    // the calendar are always in lockstep.  cycleP=0 = sunrise (~6 AM dog),
    // 0.25 = noon, 0.5 = sunset (~6 PM), 0.75 = midnight.
    const cycleP = (this._dogSecOfDay() / 86400 + 0.75) % 1;
  ...
  _drawBillboard(c,gy,night){
    ...
    // The frame ALWAYS draws so paid ad-space is always visible — the image
    // overlays it once loaded from the slideshow.
    ...
    // Billboard image stretched to fill panel (only if loaded)
    // Billboard image is a DOM <img> element (so GIFs animate).
    // Canvas only draws the panel background when no image is loaded.
    if (!this.billboardImg) {
      ...
      c.fillText('CHOCTOTV', bCX, panelT + BH/2);
    }
  ...
  _drawFence(c,gy,night){
    ...
    // ── Howliday tint: read CSS vars set by ChoctoCalendar ───────────────────
    const cs = typeof getComputedStyle !== 'undefined' ? getComputedStyle(document.documentElement) : null;
    const howlidayActive = cs ? parseInt(cs.getPropertyValue('--howliday-active') || '0', 10) : 0;
    let hLight1 = '#FFD700', hLight2 = '#C0C0C0';
    if (howlidayActive && cs) {
      hLight1 = (cs.getPropertyValue('--howliday-light-1') || '').trim() || '#FFD700';
      hLight2 = (cs.getPropertyValue('--howliday-light-2') || '').trim() || '#C0C0C0';
    }

    // Flash toggle: swap colors every ~0.5 real seconds so viewers notice
    const flashPhase = Math.floor(Date.now() / 500) % 2;

    const dc = this._fenceColor; // diamond override (still works when no howliday)

    c.save();c.shadowColor='rgba(0,0,0,.22)';c.shadowBlur=6;c.shadowOffsetX=2;c.shadowOffsetY=2;

    // Rails — white normally, tinted on howliday
    ...
    for(let x=0;x<W+PW;x+=PW+GAP){
      ...
      // ── Howliday tint overlay on top of the picket ──────────────────────
      if (howlidayActive && !dc) {
        // Alternate between the two howliday colors per picket, then flash by swapping
        const isEven = picketIdx % 2 === 0;
        const tint = (isEven ^ !!flashPhase) ? hLight1 : hLight2;
        c.save();
        c.globalAlpha = 0.55 + 0.15 * Math.sin(Date.now() / 300 + picketIdx); // subtle pulse
        c.fillStyle = tint;
        c.beginPath();c.moveTo(x,gy-PH);c.lineTo(x+PW/2,gy-PH-16);c.lineTo(x+PW,gy-PH);c.lineTo(x+PW,gy+8);c.lineTo(x,gy+8);c.closePath();c.fill();
        c.restore();
      }
  ...
  _bind(){
    ...
    // ── Billboard slideshow remote-controls (!loadboard, !boardnext) ────────
    // Ad break: hide billboard during ad, restore after
    this.EventBus.on('event:ad_break_start', () => {
      if (this._bbLayerA) { this._bbLayerA.style.opacity='0'; this._bbLayerB.style.opacity='0'; }
    });
    this.EventBus.on('event:ad_break_end', () => {
      if (this._bbFront && this.billboardImg) this._bbFront.style.opacity='1';
    });

    this.EventBus.on('event:billboard_reload', async () => {
      await this._billboardFetch();
      this._billboardIdx = 0;
      if (this._billboardAll.length) this._billboardLoadImage(this._billboardAll[0]);
      if (this._billboardTimer) clearInterval(this._billboardTimer);
      const _ms = (this.CONFIG.billboard_interval_sec || 30) * 1000;
      this._billboardTimer = setInterval(() => this._billboardNext(), _ms);
      console.log('[Billboard] reloaded by !loadboard');
    });
    this.EventBus.on('event:billboard_next', () => {
      this._billboardNext();
      console.log('[Yard] billboard advanced by !boardnext');
    });
  ...
  async _spawnWalker(msg){
    ...
    // Sprite size scales with NFT level: 130px (original) at level 1 → 400px at level 99
    const _nftLvl = Math.max(1, Math.min(99, msg.nftLevel || 1));
    const sz = Math.round(130 + 270 * (_nftLvl - 1) / 98);
  ...
  async _showPopup({title,subtitle,user,reward,balance,imgEl,pupName,rarityColor}){
    this._clearPopup();
    const col=rarityColor||'#4ecdc4';
    const img=imgEl||null;  // popup shows the WITH-background image (fav) or sprite
    const pop=document.createElement('div');
    // Center the popup exactly between the bowl pair (right edge ~192px) and the
    // morale meter (left edge ~50vw - 148px). Midpoint = 25vw + 22px.
    // translateX(-50%) makes that the popup's CENTER, not its left edge.
    pop.style.cssText=`position:fixed;bottom:206px;left:calc(25vw + 22px);transform:translateX(-50%);max-width:295px;z-index:9999;background:linear-gradient(135deg,rgba(5,5,15,.97),rgba(15,15,35,.99));border:2px solid ${col};border-radius:18px;padding:14px 16px;display:flex;align-items:center;gap:12px;min-width:230px;box-shadow:0 0 40px ${col}55,0 6px 24px rgba(0,0,0,.8);animation:yPin .3s cubic-bezier(.34,1.56,.64,1) both;`;
    if(img){const th=document.createElement('img');th.src=img.src||img;th.style.cssText='width:70px;height:70px;border-radius:8px;flex-shrink:0;object-fit:cover;border:1px solid '+col+'55;';pop.appendChild(th);}
    ...
    pop.appendChild(txt);document.body.appendChild(pop);this.popup=pop;return pop;
  },

...
module.exports = {
  name:'throw', permissions:'viewer', cooldown:60, strictCooldown:true,  // 1 per minute per user
  async execute(ctx) {
    ...
    const bonuses = SpriteManager.getBonuses(sprite.name, 'throw');
    ...
    const { amount } = calcReward('throw', cfg, {
    ...
    const resp = GameResponses.getResponse('throw', effRarity, taggedName, amount, bon);
    if (resp.emptyPaws) { say(`@${user}: ${resp.line}`); return {ok:true}; }
    say(`@${user}: ${resp.line}`);

    broadcast({ type:'game', command:'throw', user, userId, balance:newBal, reward:amount, drops,

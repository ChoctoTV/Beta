...
module.exports = {
  ...
  execute(ctx) {
    ...
    // Attach level stats if available
    try {
      const PupXP = require('../economy/pupXP');
      const stats = PupXP.getStats(db, state.favPupMint);
      if (stats) nft.level = stats.level, nft.xpPct = stats.pct, nft.xpInLevel = stats.xpInLevel, nft.xpToNext = stats.xpToNext;
    } catch {}

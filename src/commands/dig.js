...
module.exports = {
  name:'dig', permissions:'viewer', cooldown:60, strictCooldown:true,  // 1 per minute per user
  async execute(ctx) {
    ...
    const bonuses = SpriteManager.getBonuses(sprite.name, 'dig');
    ...
    const { amount } = calcReward('dig', cfg, {
    ...
    const resp = GameResponses.getResponse('dig', effRarity, taggedName, amount, bon);
    if (resp.emptyPaws) { say(`@${user}: ${resp.line}`); return {ok:true}; }
    say(`@${user}: ${resp.line}`);

    broadcast({ type:'game', command:'dig', user, userId, balance:newBal, reward:amount, drops,

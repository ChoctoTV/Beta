...
module.exports = {
  name:'onduty',
  requirePrefix: true, aliases:['offduty'], permissions:'mod', cooldown:false,
  execute(ctx) {
    const { userId, user, cmd, say, cfg, broadcast, isStr } = ctx;
    if (cmd==='offduty') {
    ...
    const role = Duty.goOnDuty(userId, isStr||false);  // streamer = dev tier

...
module.exports = {
  name:'lick', permissions:'viewer', cooldown:60, strictCooldown:true,  // 1 per minute
  execute(ctx) {
...
// Pick a random pup — rarity determines lick power
    ...
    broadcast({ type:'chest_lick', user, clicks:newClicks, lock:remaining,
                 lock_target: chest.lock_target, sprite:ctx.sprite, rarity:ctx.rarity });
    if (remaining <= 0) {
      // Pop — reward each licker equally: total licks needed ÷ number of unique lickers
      const lickers   = [..._licks.keys()];
      const reward    = Math.max(1, Math.floor(chest.lock_target / lickers.length));
      for (const uid of lickers) Balance.add(uid, reward, uid);
      const top = lickers.slice(0, 5).map(uid => `@${uid}`).join(' ');
      say(`🥜 The peanut butter jar popped open! ${chest.lock_target} licks, ${lickers.length} licker${lickers.length === 1 ? '' : 's'} — everyone gets ${reward.toLocaleString()}🍫! ${top}`);

...
// Channel point reward images: assets/channel point images/{cmd}.png (112x112 PNG)
// Drop a PNG there named after the command (e.g. toss.png) to use it as the reward icon.
const EVENTSUB_WS  = 'wss://eventsub.wss.twitch.tv/ws';
const HELIX        = 'https://api.twitch.tv/helix';

// Default channel point cost per action (overridden by Config cp_cost_* keys)
const DEFAULT_COSTS = {
  toss:1, throw:1, dig:1, fish:1, walk:1, lurk:1,
  lick:1, battle:1, setwallet:1, setfav:1, gbm:1, gauntlet:1,
};
...
const TITLE_MAP = {
  ...
  'choctotv lurk':      'lurk',
  'choctotv lick':      'lick',
  'choctotv battle':    'battle',
  'choctotv setwallet': 'setwallet',
  'choctotv setfav':    'setfav',
  'choctotv gbm':       'gbm',
  'choctotv gauntlet':  'gauntlet',
};
...
async function createRewards(broadcasterId, token, clientId, cfg) {
  for (const [cmd, cost] of Object.entries(DEFAULT_COSTS)) {
    ...
    if (!exists) {
      ...
      const r = await api('POST', '/channel_points/custom_rewards', token, clientId, body);
      ...
      try {
        ...
        if (type === 'session_welcome') {
          ...
          if (subType === 'channel.ad_break.begin') {
            ...
            if (cmd && _dispatch) _dispatch(
              evt.user_id,
              evt.user_name || evt.user_login,
              cmd,
              evt.reward?.id,
              evt.id,             // redemption ID
              evt.user_input || '' // text input if reward requires it
            );
          }
...
/**
 * Fulfill a redemption — marks it FULFILLED so user knows it worked.
 * Requires channel:manage:redemptions scope.
 */
async function fulfill(broadcasterId, rewardId, redemptionId, token, clientId) {
  ...
  try {
    await api('PATCH',
      ...
      token, clientId, { status: 'FULFILLED' }
    );
  } catch {}
}

/**
 * Cancel a redemption — refunds the user's channel points.
 * Requires channel:manage:redemptions scope.
 */
async function cancel(broadcasterId, rewardId, redemptionId, token, clientId) {
  if (!rewardId || !redemptionId) return;
  try {
    await api('PATCH',
      `/channel_points/custom_rewards/redemptions?broadcaster_id=${broadcasterId}&reward_id=${rewardId}&id=${redemptionId}`,
      token, clientId, { status: 'CANCELED' }
    );
  } catch {}
}

// Expose broadcasterID so app.js can pass it to fulfill/cancel
let _broadcasterId = null;

module.exports = { connect, disconnect, fulfill, cancel, getBroadcasterId: () => _broadcasterId };

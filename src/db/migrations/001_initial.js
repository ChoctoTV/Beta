...
module.exports = {
  ...
  up(db) {
    ...
    // ── XP tables ────────────────────────────────────────────────────────────
    db.prepare(`CREATE TABLE IF NOT EXISTS pup_xp (
      mint         TEXT PRIMARY KEY,
      xp           INTEGER NOT NULL DEFAULT 0,
      stored_level INTEGER DEFAULT 1,
      last_owner   TEXT,
      created_at   DATETIME DEFAULT (datetime('now')),
      updated_at   DATETIME DEFAULT (datetime('now'))
    )`).run();
    try { db.prepare('ALTER TABLE pup_xp ADD COLUMN last_owner TEXT').run(); } catch {}
    try { db.prepare('ALTER TABLE pup_xp ADD COLUMN stored_level INTEGER DEFAULT 1').run(); } catch {}

    db.prepare(`CREATE TABLE IF NOT EXISTS user_xp (

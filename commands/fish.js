...
module.exports = {
  name:'fish', permissions:'viewer', cooldown:true,
  async execute(ctx) {
    const { userId, user, say, cfg, broadcast, Balance, Inventory, PupCore, Lurk, SpriteManager, CHANNEL, isLurkTick } = ctx;

    try {
      const fishMod = require('./fish'); // self-reference for recordWalker/getRecent
      const recents = typeof fishMod.getRecent === 'function' ? fishMod.getRecent(userId) : [];
      var catchFav  = (recents.length && Math.random() < 0.01) ? recents[Math.floor(Math.random()*recents.length)] : null;
    } catch { var catchFav = null; }
    ...
    const fishMult = catchFav ? 2 : 1;
    const { amount } = calcReward('fish', cfg, {
      usePC, favGame:bonuses.favGame, favWeather:bonuses.favWeather,
      modOnDuty:Duty.modBoost(cfg)>0, rarity:sprite.rarity, lurkMult:fishMult });
    ...
    if (catchFav) {
      say(`@${user}: ${taggedName} hooked ${catchFav.userId}'s ${catchFav.name||'pup'}! 2x REWARD +${amount.toLocaleString()}🍫!`);
    } else {
      const resp = GameResponses.getResponse('fish', sprite.rarity, taggedName, amount, bon);
